...
function useCinematicReveal() {
  useEffect(() => {
    ...
    // Observe all cin-scene elements
    document.querySelectorAll(".cin-scene").forEach((el) => observer.observe(el));
...
function HeroSlider() {
  ...
  useEffect(() => {
    ...
    // Hide letterbox bars after opening sequence
    const t = setTimeout(() => setShowLetterbox(false), 1800);
    return () => { window.removeEventListener("storage", onStorage); window.removeEventListener("hero_slides_updated", onUpdate); clearTimeout(t); };
  }, []);
  ...
  return (
    ...
    >
      {/* ── CINEMATIC LETTERBOX BARS (open sequence) ── */}
      <div style={{
        position: "absolute", top: 0, left: 0, right: 0, height: 64,
        background: "#000", zIndex: 20,
        transformOrigin: "top",
        transition: "transform 1.2s cubic-bezier(.16,1,.3,1)",
        transform: showLetterbox ? "scaleY(1)" : "scaleY(0)",
        pointerEvents: "none",
      }} />
      <div style={{
      ...
      <div style={{
        ...
        opacity: 0.04,
        animation: "cin-grain 0.15s steps(1) infinite",
        mixBlendMode: "overlay",
      }} />

      {/* ── CINEMATIC VIGNETTE on hero ── */}
      <div style={{
        position: "absolute", inset: 0, zIndex: 5, pointerEvents: "none",
        background: "radial-gradient(ellipse at center, transparent 40%, rgba(0,0,0,.55) 100%)",
      }} />

      {/* Green shimmer while loading */}
      <div style={{
      ...
      {slides.map((sl, i) => (
        <div key={sl.id} style={{
          ...
          {sl.image && (
            <img
              ...
              style={{
                ...
                /* Ken Burns camera pan on active slide */
                animation: i === current ? "cin-ken-burns 12s ease-in-out infinite" : "none",
          ...
          <div style={{
            position: "absolute", inset: 0,
            background: "linear-gradient(to right, rgba(0,0,0,.68) 0%, rgba(0,0,0,.28) 50%, rgba(0,0,0,.05) 80%, transparent 100%)",
          }} />
          <div style={{
      ...
      {/* Gold top accent */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(to right, transparent, ${s.accent}cc, transparent)`, zIndex: 7 }} />

      {/* Floating leaf */}
      <svg style={{ position: "absolute", right: "8%", top: "18%", zIndex: 7, opacity: 0.15, animation: "ab-rotateleaf 4s ease-in-out infinite" }} width="90" height="90" viewBox="0 0 100 100">
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            animation: "cin-text-reveal .8s ease both",
          }}>
          ...
          <h1 className="ab-serif" style={{
            ...
            animation: "cin-text-reveal .9s ease .1s both",
          }}>
            {s.title}<br />
            <span style={{
          ...
          <p style={{ color: "rgba(255,255,255,.85)", fontSize: "1rem", lineHeight: 1.72, maxWidth: 400, marginBottom: 30, animation: "cin-text-reveal .9s ease .2s both" }}>{s.subtitle}</p>

          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, animation: "cin-text-reveal .9s ease .3s both" }}>
      ...
      {/* Counter */}
      <div style={{ position: "absolute", top: 22, right: 18, zIndex: 9, color: "rgba(255,255,255,.4)", fontSize: 12, fontWeight: 700, fontFamily: "monospace", letterSpacing: 2 }}>
        {String(current + 1).padStart(2, "0")} / {String(slides.length).padStart(2, "0")}
      </div>

      {/* Dots */}
      <div style={{ position: "absolute", bottom: 22, left: "50%", transform: "translateX(-50%)", zIndex: 9, display: "flex", gap: 7, alignItems: "center" }}>
        {slides.map((sl, i) => (
...
function CategorySection({ categories, isLoading }: { categories: any[]; isLoading: boolean }) {
  ...
  return (
    <section className="cin-scene" style={{ background: BG, paddingBottom: 8 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", padding: "22px 16px 14px" }}>
        <div>
          <p className="ab-label"><IconLeafSmall /> Explore categories</p>
          <h2 className="ab-serif cin-heading" style={{ fontSize: "1.35rem", fontWeight: 700, color: "#1B3A22", margin: 0, lineHeight: 1.2 }}>
          ...
          : cats.map((cat: any) => {
              ...
              return (
                <Link key={cat.id ?? cat.slug} href={`/category/${cat.slug}`}
                  className="cin-cat-card"
                  style={{ flexShrink: 0, width: 148, borderRadius: 20, overflow: "hidden", textDecoration: "none", scrollSnapAlign: "start", background: "#fff", border: `0.5px solid rgba(212,175,55,0.22)`, boxShadow: "0 4px 18px rgba(27,94,32,0.10)", display: "block" }}>
                  <div style={{ width: "100%", height: 158, overflow: "hidden", position: "relative", background: "#edf5ef" }}>
                    {imgSrc
                      ? <img src={imgSrc} alt={cat.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "transform .6s cubic-bezier(.16,1,.3,1)" }} onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
...
function FeaturedCarousel({ products, isLoading }: { products: any[]; isLoading: boolean }) {
  ...
  return (
    <section className="cin-scene" style={{ padding: "24px 0 20px", background: BG }}>
      <div style={{ padding: "0 16px", display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          ...
          <h2 className="ab-h2 cin-heading" style={{ fontSize: "1.5rem" }}>Featured <span>Products</span></h2>
        </div>
      ...
      <div ref={scrollRef} className="ab-noscroll" style={{ display: "flex", gap: GAP, overflowX: "auto", padding: "4px 16px 8px", scrollSnapType: "x mandatory" }} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
        {isLoading
          ...
          : products.map(p => (
            <div key={p.id} className="cin-product-wrap" style={{ width: CARD_W, flexShrink: 0, scrollSnapAlign: "start" }}>
...
function NewArrivalsSection() {
  ...
  if (!products.length) return <div style={{ textAlign: "center", padding: "36px 0", color: "#9ca3af" }}><p style={{ fontSize: 13 }}>New arrivals coming soon!</p></div>;
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      {products.map((p: any, i: number) => (
        <div key={p.id} className={`cin-product-wrap cin-scene cin-scene-delay-${(i % 3) + 1}`}>
          <ProductCard product={p} />
        </div>
      ))}
    </div>
...
function TestimonialsSection() {
  ...
  const CARD_W = 240, GAP = 14, STEP = 254;
  ...
  return (
    <section className="cin-scene" style={{ padding: "28px 0 24px", overflow: "hidden", position: "relative", backgroundImage: "url('https://swift-gold-votdrc5f.edgeone.dev/3fd87767-a435-46dd-896f-385a0e1341a4.png')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed" }} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
        ...
        {reviews.map((r, idx) => (
          <div key={r.name} className="ab-test-card" style={{ flexShrink: 0, width: CARD_W, background: r.bg, borderRadius: 20, padding: "18px 16px 16px", border: idx === active ? `1px solid ${GOLD}66` : "1px solid rgba(0,0,0,0.06)", boxShadow: idx === active ? "0 10px 28px rgba(201,168,76,0.22)" : "0 2px 12px rgba(0,0,0,0.06)", transform: idx === active ? "translateY(-3px) scale(1.015)" : "translateY(0) scale(1)", scrollSnapAlign: "start", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              ...
              {r.verified && <div style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 8.5, fontWeight: 700, color: G, background: "rgba(26,90,50,0.1)", borderRadius: 100, padding: "2px 7px" }}><svg width="9" height="9" viewBox="0 0 24 24" fill={G}><path d="M9 16.2l-3.5-3.5L4 14.2l5 5 11-11-1.5-1.5z"/></svg>VERIFIED</div>}
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <p style={{ fontWeight: 800, fontSize: 13, color: "#1a2d1a", margin: 0 }}>{r.name}</p>
                <p style={{ fontSize: 10.5, color: "#888", margin: 0 }}>{r.city}</p>
              </div>
            </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 4, marginBottom: 16, position: "relative", zIndex: 2 }}>
        {reviews.map((_, i) => <button key={i} onClick={() => goTo(i)} style={{ width: i === active ? 18 : 6, height: 6, borderRadius: 4, background: i === active ? G : "#d8cfc0", border: "none", cursor: "pointer", padding: 0, transition: "all .3s" }} />)}
      </div>
...
function RecentlyViewedSection() {
  ...
  return (
    <section className="cin-scene" style={{ padding: "22px 16px 24px", background: "#fff" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          <p className="ab-label"><IconLeafSmall /> RECENTLY VIEWED</p>
          <h2 className="ab-h2 cin-heading" style={{ fontSize: "1.25rem" }}>Continue Browsing</h2>
        </div>
        <Link href="/products" className="ab-viewall" style={{ fontSize: 12 }}>See all <IconArrowRight size={13} /></Link>
      </div>
      {isLoading
        ? <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>{[1, 2].map(i => <Skeleton key={i} style={{ height: 220, borderRadius: 18 }} />)}</div>
        : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>{(productsData ?? []).map((p: any) => <div key={p.id} className="cin-product-wrap"><ProductCard product={p} /></div>)}</div>
      }
    </section>
  );
}
...
export default function Home() {
  ...
  // Activate cinematic reveal observer after mount
  useCinematicReveal();
  ...
  return (
    <div className="ab-root">
      ...
      {/* ── GLOBAL FILM GRAIN (subtle, always on) ── */}
      <div className="cin-grain-overlay" aria-hidden="true" />
      ...
      <section className="cin-scene" style={{ padding: "24px 16px 20px", background: BG }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            <p className="ab-label"><IconLeafSmall /> NEW ARRIVALS</p>
            <h2 className="ab-h2 cin-heading">Latest From Our <span>Artisans</span></h2>
          </div>
