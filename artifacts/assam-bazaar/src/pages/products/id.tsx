    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* Sticky bottom */
        .pdp-sticky { position: fixed; bottom: 0; left: 0; right: 0; z-index: 100; background: #fff; border-top: 1px solid #eee; padding: 10px 16px; display: flex; gap: 10px; }
      ...
      {isLoading ? <LoadingSkeleton /> : !product ? (
        ...
        <div className="pdp-wrap pdp-fadein">
          ...
          <div className="pdp-info">
            ...
            {product.artisan && (
              <p className="pdp-artisan">
                By <strong>{product.artisan}</strong>{product.origin && ` · ${product.origin}`}
              </p>
            )}
          ...
          {product.stock > 0 ? (
            <>
              ...
              <div className="pdp-cta">
                ...
                href={`https://wa.me/${import.meta.env.VITE_WHATSAPP_NUMBER?.replace(/\D/g, "") ?? "919395722454"}?text=${encodeURIComponent(`Hi! I want to order: ${product.name} (₹${product.price})\n${window.location.href}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ height: 52, background: "#25D366", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 800, color: "#fff", fontFamily: "'Nunito',sans-serif", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, textDecoration: "none" }}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347zM12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.117 1.528 5.847L0 24l6.326-1.506A11.926 11.926 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.797 9.797 0 01-4.989-1.367l-.358-.213-3.716.885.917-3.617-.233-.372A9.79 9.79 0 012.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg>
                  Order on WhatsApp
                </a>
